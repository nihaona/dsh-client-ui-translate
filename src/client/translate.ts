...
export type LangCode =
  ...
  | 'ru' | 'it' | 'pt-BR' | 'ar' | 'th' | 'vi' | 'id' | 'tr' | 'nl' | 'pl' | 'uk' | 'hi'
 ...
 */
export function detectLanguage(text: string): ResolvedLang {
  let kana = 0
  let hangul = 0
  let hanzi = 0
  let cyrillic = 0
  ...
  for (const char of text) {
    ...
    else if (code >= 0x4e00 && code <= 0x9fff) hanzi += 1
 ...
 */
function lastChunkBoundary(slice: string): number {
  let sentenceEnd = -1
  let space = -1
...
export class TranslateFailure extends Error {
  constructor(readonly kind: TranslateFailureKind, message: string) {
    super(message)
    this.name = 'TranslateFailure'
  }
}

/** The subset of the MyMemory response the panel reads. */
...
export async function translateText(
  text: string,
  source: LangCode,
  target: Exclude<LangCode, 'auto'>,
  signal: AbortSignal,
): Promise<{ translated: string; sourceUsed: ResolvedLang }> {
  const trimmed = text.trim()
  if (trimmed.length === 0) throw new TranslateFailure('empty', 'empty input')
  const sourceUsed: ResolvedLang = source === 'auto' ? detectLanguage(trimmed) : source
